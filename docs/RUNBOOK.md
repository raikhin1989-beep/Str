...
Бэкап фото живёт на том же сервере, что и сами фото, — от потери сервера он
не спасает. Чтобы унести альбом целиком:

```
curl -u admin:ПАРОЛЬ -O -J https://raikhin.duckdns.org/api/admin/photos.zip
curl -u admin:ПАРОЛЬ -O -J 'https://raikhin.duckdns.org/api/admin/photos/collage.jpg?cell=800'
```

То же самое кнопками — в админке, раздел «Фотоальбом». В архиве лежат
полноразмерные файлы и `photos.csv` с именами гостей и подписями: в именах
файлов подпись обрезана, в CSV она целиком.

`cell` — сторона клетки коллажа в пикселях, 120–1000. Если сетка выходит
больше 8000 px по стороне, запрос отвечает 400 с этим же объяснением.
