...
- `telegram.last_poll_at` старше часа → задача опроса не ходит, привязка
  гостей встала. Это отметка **самого опроса**, она обновляется каждые
  полчаса даже когда обновлений не было;
- `telegram.last_update_at` — когда в последний раз приходило обновление от
  гостя. Может быть старым или пустым, и это **норма**: «Старт» жмут редко.
  Тревожиться по нему не нужно.
...
2. `.github/workflows/deploy.yml` — `SITE_DOMAIN_FIXED`.
3. `.github/workflows/telegram.yml` — **три места**: `SITE` в шагах опроса и
   рассылки и адрес в шаге напоминаний. Задача ходит на сервер снаружи, и
   со старым именем она молча перестанет работать: бот отвалится, а деплой
   при этом будет зелёным.
4. `backend/app.py` — `SITE_URL` (подставляется в ссылки в сообщениях бота).
   Значение читается из окружения, так что альтернатива — задать `SITE_URL`
   в systemd-юните, но по умолчанию берётся строка из кода.
5. `site/index.html` — `og:url` **и** `og:image`.
6. `scripts/make_ics.py` — `SITE_URL`, затем перегенерировать
   `site/calendar.ics`.
7. Запушить. Caddy выпустит сертификат сам.

Проверка, что ничего не забыли:

```
grep -rn 'raikhin.duckdns.org' site/ backend/ scripts/ .github/
```
